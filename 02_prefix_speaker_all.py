...
custom_stopwords = {'yeah', 'yep', 'think', 'say', 'like', 'really', 'ok', 'use', 'im', 'sprecher', 'space', 'yes', 'actually', 'know'}  # Custom list of overused words to remove
...
def preprocess_text(text):
    ...
    tokens = [lemmatizer.lemmatize(token) for token in tokens if token not in stop_words and not re.match(r'sprecher\d+', token)]
    return ' '.join(tokens)
...
fig.write_image("figs/02_prefix_topics.png")
...
try:
    if embeddings.shape[0] > 1:
        ...
        plt.savefig("figs/02_prefix_umap.png")
    else:
...
filtered_topics.to_csv('output/02_refined_topics.csv', index=False)
...
try:
    topic_model.visualize_documents(sentences, embeddings=embeddings).write_html("figs/02_prefix_documents.html")
except ValueError:
